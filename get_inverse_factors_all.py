"""
Created on Tue Jun 25 07:56:54 2024
...
# In[]
def get_XtX_inv(factor_ret, factor_ThD, size, order, pen_lbd, cache, cache_her):
    ...
    for i in range(factor_ret.shape[1]):  # range(factor_ret.shape[1])   
        try:            
            factor_name = factor_ret.columns[i]           
            # factor_name =factor_ret.columns.get_loc('USAADP') 
            ...
            if factor_name.startswith("USA"):
                ...
                if cache_entry != None:
                    ...
                    X_her_kfolds =X_her_kfolds|cache_entry_her
                    continue
                else:                    
                    cache_entry = dict()  
            ...
            for f, (train_index, test_index) in enumerate(kf.split(X_All)):
                ...
                factor_thresholds =factor_ThD.loc[f,cols_ThD]
                                
                x = X_All.iloc[train_index]                
                X = adj_hermite(x, order, factor_thresholds)
...
def main(portfolio_date, options):
    ...
    if common_factor_file.exists():
        ...
        np.save(  F_folder / "Q_size36_order4_adj", M )
        return
...
# In[]
    ...
    #     F_folder_stock = os.path.join(F_folder, filename) #'C:\\Users\\ecoramy\\my_wdc\\Polymodel-v3\\portfolios\\2021-03-31\\2_dataProcessed\\Factors\\AAPL'
    #     # checking if it is a file
    #     if os.path.isfile(F_folder_stock):
    #         continue
             
    #     print( Path(os.path.basename(filename)).with_suffix(''), end=",") # output: AAPL
    #     #print("Loading data start for stock {0}".format(filename))
...
# In[]         
        ...
        #M = get_XtX_inv(factor_ret, size=36, order=4, pen_lbd=np.arange(0., 10.05, 0.5), cache=cache) 
        M = get_XtX_inv(factor_ret, factor_ThD, size=36, order=4, pen_lbd=np.arange(0.1, 10.50, 0.5), cache=cache, cache_her=cache_her) 
        
        #np.save(R"C:\Users\ecoramy\Documents\Eric_new_polymodel"+R"\Q_size36_order4_adj_"+startdate+'_'+enddate, M)
        
        if options.get('compressed_pickle', False):
            compressed_pickle.compressed_pickle( os.path.join( F_folder_stock, 'XtX_inv_order4_kfolds'), M )
        else:
            np.save( os.path.join( F_folder_stock, 'XtX_inv_order4_kfolds'), M[0] )
            np.save( os.path.join( F_folder_stock, 'X_adjHer_order4_kfolds'), M[1] )
