...
def main(portfolio_date, options):
    ...
    for i, asset_ticker in enumerate(common.stock_tickers):
        ...
        genome = Genome(standardized_returns, lead_lag=1, signature_level=2, best_ga_file=best_ga_file)    
        population_size = 100
        num_generations = 100              
...
class Genome:
    ...
    def __init__(self,standardized_returns, lead_lag=1, signature_level=2, chromosone_length=5, best_ga_file=None  ):
    ...
    def fitness_func(self, ga_instance, solution, solution_idx):
        ...
        # r_r_r =get_sig_functions.leadlag_embedding(df, ll =1) # time + lead lag agumentation
        
        window_size = 5
        # sw_gen = sliding_window(r_r,window_size)
        sw_gen = expand_window(r_r,window_size)
        X = [w for w in sw_gen]
        ...
        # sig_dimension  =int( (  len(r[0]) **(self.signature_level +1)  -1  )  /(len(r[0])  -  1)  -1 )
